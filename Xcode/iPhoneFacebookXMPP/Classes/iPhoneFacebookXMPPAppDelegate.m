...
@implementation iPhoneFacebookXMPPAppDelegate
...
+ (iPhoneFacebookXMPPAppDelegate *)sharedAppDelegate
{
	return (iPhoneFacebookXMPPAppDelegate *)[[UIApplication sharedApplication] delegate];
}
...
//	xmppRosterStorage = [[XMPPRosterCoreDataStorage alloc] initWithInMemoryStore];
	...
	// Activate optional xmpp modules
    // If you already downloaded user profile pics using Graph, don't use the vCard modules
    // If you are only sending messages, don't use the Roster module
...
{    
    ...
    self.settingsViewController.statusLabel.text = @"XMPP connecting...";
	...
	if (![xmppStream connect:&error])
	{
        ...
        self.settingsViewController.statusLabel.text = @"XMPP connect failed";
	}
}
...
{
	DDLogVerbose(@"%@: %@", THIS_FILE, THIS_METHOD);
    self.settingsViewController.statusLabel.text = @"Facebook login failed";
}
...
{
    DDLogVerbose(@"%@: %@", THIS_FILE, THIS_METHOD);
    self.settingsViewController.statusLabel.text = @"Logged out";
}
...
- (void)xmppStreamDidConnect:(XMPPStream *)sender
{
    ...
    {
        self.settingsViewController.statusLabel.text = @"XMPP STARTTLS...";
        NSError *error = nil;
        ...
        if (result == NO)
        {
            ...
            self.settingsViewController.statusLabel.text = @"XMPP STARTTLS failed";
        }
    } 
    else 
    {
        self.settingsViewController.statusLabel.text = @"XMPP X-FACEBOOK-PLATFORM SASL...";
        NSError *error = nil;
        ...
        if (result == NO)
        {
            ...
            self.settingsViewController.statusLabel.text = @"XMPP authentication failed";
        }
    }
}
...
{
	DDLogVerbose(@"%@: %@", THIS_FILE, THIS_METHOD);
    self.settingsViewController.statusLabel.text = @"XMPP authenticated";

	// Only go online if you want to appear Online to friends
    [self goOnline];
}
...
- (void)xmppStreamDidDisconnect:(XMPPStream *)sender withError:(NSError *)error
{
	...
	DDLogVerbose(@"%@: %@", THIS_FILE, THIS_METHOD);
    self.settingsViewController.statusLabel.text = @"XMPP disconnected";
}
