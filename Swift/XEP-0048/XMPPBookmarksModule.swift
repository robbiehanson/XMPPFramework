...
public class XMPPBookmarksModule: XMPPModule {
    ...
    @discardableResult public override func activate(_ xmppStream: XMPPStream) -> Bool {
    ...
    @objc public func fetchBookmarks(_ completion: @escaping (_ bookmarks: [XMPPBookmark]?, _ responseIq: XMPPIQ?) -> Void, completionQueue: DispatchQueue? = nil) {
        ...
        let completionHandler = { (_ bookmarks: [XMPPBookmark]?, _ responseIq: XMPPIQ?) -> Void in
        ...
        let iqHandler = { (_ obj: Any, _: XMPPTrackingInfo) in
    ...
    @objc public func fetchAndPublish(bookmarksToAdd: [XMPPBookmark], bookmarksToRemove: [XMPPBookmark]? = nil, completion: @escaping (_ bookmarks: [XMPPBookmark]?, _ responseIq: XMPPIQ?) -> Void, completionQueue: DispatchQueue? = nil) {
        // Executes completion block on proper queue
        let completionHandler = { (_ bookmarks: [XMPPBookmark]?, _ responseIq: XMPPIQ?) -> Void in
            if let completionQueue = completionQueue {
                completionQueue.async {
    ...
    @objc public func publishBookmarks(_ bookmarks: [XMPPBookmark], completion: @escaping (_ success: Bool, _ responseIq: XMPPIQ?) -> Void, completionQueue: DispatchQueue? = nil) {
        let storage = XMLElement.storage
        let query = XMLElement.query(child: storage)
        ...
        // Executes completion block on proper queue
        let completionHandler = { (_ success: Bool, _ responseIq: XMPPIQ?) -> Void in
            if let completionQueue = completionQueue {
                completionQueue.async {
...
extension XMPPBookmarksModule: XMPPStreamDelegate {
    public func xmppStream(_: XMPPStream, didReceive iq: XMPPIQ) -> Bool {
...
private struct PrivateXmlStorageConstants {
